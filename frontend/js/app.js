...
async function changeTaskStatus(id, taskIndex) {
	...
	try {
		...
		// With status update, need to invoke getTaskPoints() to update total points earned 
		let total = await contract.methods.getTaskPoints().call({ from: web3.eth.defaultAccount });
	    document.getElementById("totalTaskPoints").innerText = total + " Completed Task Points Earned";
...
async function addTask(name,points) {
	...
	document.getElementById("task-points").value = 0;
	...
	contract.methods
		...
		.then(
			(numberOfTask) => {
				// Add the task to the HTML and update the task count// how does this work?
				addTaskToList(numberOfTask, name, points, false); 
				updateTasksCount();
			},
			(err) => {
	...
	try {
		await contract.methods.addTask(name,points).send({ from: web3.eth.defaultAccount });
		console.log("Add task " + name + points + " to blockchain");
		let total = await contract.methods.getTaskPoints().call({ from: web3.eth.defaultAccount });
	    document.getElementById("totalTaskPoints").innerText = total + " Completed Task Points Earned";
...
function addTaskToList(id, name, points, status) {
	console.log("addTaskToList(): Add Task " + id + " " + [name, points, status]);
	...
	*/
	...
	let task = document.createTextNode(name + "  task earns  " + points + "  Points");
